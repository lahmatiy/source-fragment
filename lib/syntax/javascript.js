var keywordRx = new RegExp('\\b(' + [
    'break', 'case', 'catch', 'continue', 'default', 'delete', 'do',
    'else', 'false', 'for', 'function', 'if', 'in', 'instanceof',
    'new', 'null', 'return', 'super', 'switch', 'this', 'throw',
    'true', 'try', 'typeof', 'var', 'while', 'with', 'let', 'const',
    'of', 'class', 'extends', 'yield', 'async', 'await'
].join('|') + ')\\b', 'g');

module.exports = function(text, wrap, rangeStart, rangeEnd, rangeName) {
    function addMatch(kind, start, end, rn) {
        if (lastMatchPos !== start) {
            result.push(text.substring(lastMatchPos, start).replace(keywordRx, wrap('token-keyword', '$1')));
        }
    ...
    if (resultRangeStart !== -1) {
        if (!resultRangeEnd) {
            ...
            result += rangeWrapper[1];
        }

        result =
            result
                .substr(0, resultRangeStart) +
            result
                .substring(resultRangeStart, resultRangeEnd)
                .replace(/\n/g, rangeWrapper[1] + '\n' + rangeWrapper[0]) +
            result
                .substr(resultRangeEnd);
    }
